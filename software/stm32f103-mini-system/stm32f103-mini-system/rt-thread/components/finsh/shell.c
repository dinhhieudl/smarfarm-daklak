...
//		static rt_device_t vcom_serial;              //修改 20240416 修改使用USB虚拟串口时 增加
//     vcom_serial = rt_device_find("vcom");       //修改 20240416 修改使用USB虚拟串口时 增加
//		if(vcom_serial)
//		{	
//			rt_device_open(vcom_serial,RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX);
//    }
    while (1)
    {
//			  if(vcom_serial)
//				{		
//      	    if(!rt_device_read(vcom_serial, -1, &ch, 1)) //修改 20240416 修改使用USB虚拟串口时 增加
//          {
//					  continue;
//					}
//				}
       ch = finsh_getchar();								   //修改 20240416 修改使用USB虚拟串口时  屏蔽
        if (ch < 0)
        {
            continue;
        }
