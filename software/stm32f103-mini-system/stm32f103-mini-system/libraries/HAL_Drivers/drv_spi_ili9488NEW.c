...
#define LCD_DC_PIN            GET_PIN(A, 15)

#define LCD_DEVICE(dev)     (struct drv_lcd_device*)(dev)

#define LCD_CLEAR_SEND_NUMBER (LCD_H * LCD_W /10)
...
int rt_hw_spi_lcd_init(void)
{
    ...
    rt_hw_spi_device_attach("spi3", "spi30", GPIOD, GPIO_PIN_2);
    lcd_gpio_init();
...
void lcd_clear(rt_uint32_t color)
{
    ...
    {
        /* color is 16 bit */
        for (j = 0; j < LCD_CLEAR_SEND_NUMBER / 3; j++)
        {
            buf[j * 3] =  data[0];
            buf[j * 3 + 1] =  data[1];
            buf[j * 3 + 2] =  data[2];
...
// * @return  void
// */
static rt_uint8_t r6,g6,b6;
void lcd_fill_array(rt_uint16_t x_start, rt_uint16_t y_start, rt_uint16_t x_end, rt_uint16_t y_end, lv_color_t *pcolor)
{
    rt_uint32_t size = 0;
    ...
    {			                               
				r = (pcolor->full >> 11) & 0x1F;
				g = (pcolor->full >> 5)  & 0x3F;
				b =  pcolor->full        & 0x1F;
			  r6 = (r << 3) | (r >> 4);
			  g6 = g<<2;
			  b6 = (b << 3) | (b >> 4);  
...
static rt_err_t drv_lcd_control(struct rt_device *device, int cmd, void *args)
{
    ...
    case RTGRAPHIC_CTRL_RECT_UPDATE:
    {
        ...
        {
            lcd_show_image(0, 0, LCD_WIDTH, LCD_HEIGHT, _lcd.lcd_info.framebuffer);
        }
    }
    break;
...
int drv_lcd_ili9488_hw_init(void)
{
    ...
    lcd_show_string(0, 0, 32, "Hello, RT-Thread!");
...
//   lcd_show_image(0, 0, 240, 69, image_rttlogo);
    ...
    lcd_show_string(0, 69+16+24, 32, "He Bei Zhan Huang Dian Zi");


    rt_thread_mdelay(200);

}
INIT_APP_EXPORT(ili9488_test);
